...
const AnimationSlider = ({ animations }) => {
  ...
  const handlePrevClick = () => {
    setCurrentSlide((currentSlide) =>
      currentSlide === 0 ? animations.length - 1 : currentSlide - 1
    );
  };

  const handleNextClick = () => {
    setCurrentSlide((currentSlide) => (currentSlide + 1) % animations.length);
  };
  ...
  return (
    <>
      <StSliderHeader>
        <StMoviecategory>애니메이션</StMoviecategory>
      ...
      <StContainer>
        ...
        <StSlideContainer translateX={-currentSlide * 100}>
          {animations?.map((animation) => (
            <StSlideThum
              key={animation.contentIdx}
              src={animation.videoThumUrl}
              onClick={() => onClickThum(animation.contentIdx)}
            />
          ))}
        </StSlideContainer>
        <StArrow
