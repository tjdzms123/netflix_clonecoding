...
const ComedySlider = ({ comedies }) => {
  ...
  const handlePrevClick = () => {
    setCurrentSlide((currentSlide) =>
      currentSlide === 0 ? comedies.length - 1 : currentSlide - 1
    );
  };

  const handleNextClick = () => {
    setCurrentSlide((currentSlide) => (currentSlide + 1) % comedies.length);
  };
  ...
  return (
    <>
      <StSliderHeader>
        <StMoviecategory>코미디</StMoviecategory>
      ...
      <StContainer>
        ...
        <StSlideContainer translateX={-currentSlide * 100}>
          {comedies?.map((comedy) => (
            <StSlideThum
              key={comedy.contentIdx}
              src={comedy.videoThumUrl}
              onClick={() => onClickThum(comedy.contentIdx)}
            />
          ))}
        </StSlideContainer>
        <StArrow
